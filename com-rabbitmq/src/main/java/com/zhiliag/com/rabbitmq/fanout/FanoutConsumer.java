...
public class FanoutConsumer {
    ...
    @RabbitListener(queues = "${spring.rabbitmq.queue.fanout-name1}",containerFactory = "singleListenerContainer")
    public void consumerFanoutOne(@Payload byte[] bytes){
        try{
//            EventInfo eventInfo=objectMapper.readValue(bytes,EventInfo.class);
            logger.info("消息模型-fanoutExchange-消费端1-接收到的消息为:{}",bytes);
        }catch (Exception e){
            logger.error("消费模型-fanoutExchange-消费端1-接受到的消息异常",e.fillInStackTrace());
        }
    }

    @RabbitListener(queues = "${spring.rabbitmq.queue.fanout-name2}",containerFactory = "singleListenerContainer")
